  ...
  for (int directionID = 0; directionID <= 1; ++directionID) {
    for (auto padIDToFill : testCases) {
      ...
      const std::vector<BeamCalCluster>& clusters0 =
          signalBeamCals[0].lookForNeighbouringClustersOverSigma(backgroundBeamCals[0], cuts, true);
      const std::vector<BeamCalCluster>& clusters1 =
          signalBeamCals[1].lookForNeighbouringClustersOverSigma(backgroundBeamCals[1], cuts, true);
