...
int reconstructBecas (int argn, char **argc) {
  ...
  const std::vector<BeamCalCluster>& clusters0 = signalBeamCals[0].lookForNeighbouringClustersOverSigma(backgroundBeamCals[0], cuts, false);
  const std::vector<BeamCalCluster>& clusters1 = signalBeamCals[1].lookForNeighbouringClustersOverSigma(backgroundBeamCals[1], cuts, false);
